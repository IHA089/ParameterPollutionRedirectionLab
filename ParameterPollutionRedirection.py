...
@ParameterPollutionRedirection.route('/')
def home():
    if not check_cookies():
        session.clear()
    return render_template('index.html', user=session.get('user'))

@ParameterPollutionRedirection.route('/index.html')
def index_html():
    if not check_cookies():
        session.clear()
    return render_template('index.html', user=session.get('user'))

@ParameterPollutionRedirection.route('/login.html')
def login_html():
    if not check_cookies():
        session.clear()
...
@ParameterPollutionRedirection.route('/join.html')
def join_html():
    if not check_cookies():
        session.clear()
    return render_template('join.html')

@ParameterPollutionRedirection.route('/forgot-password.html')
...
@ParameterPollutionRedirection.route('/acceptable.html')
def acceptable_html():
    if not check_cookies():
        session.clear()
...
@ParameterPollutionRedirection.route('/privacy.html')
def privacy_html():
    if not check_cookies():
        session.clear()
...
@ParameterPollutionRedirection.route('/confirm', methods=['POST'])
def confirm():
...
@ParameterPollutionRedirection.route('/resend', methods=['POST'])
def resend():
    username = request.form.get('username')
    password = request.form.get('password')
    hash_password=hashlib.md5(password.encode()).hexdigest()
    ...
    if code:
        ...
        payload = {"email": username,
                    "sender":"IHA089 Labs ::: ParameterPollutionRedirectionLab",
                    "subject":"ParameterPollutionRedirectionLab::Verify Your Accout",
...
@ParameterPollutionRedirection.route('/login', methods=['GET', 'POST'])
def login():
    if request.method == 'GET':
        ...
        if user_uuid in user_data and jwt_token == user_data[user_uuid]:
            decoded = pyjwt.decode(jwt_token, JWT_SECRET, algorithms="HS256")
            session['user'] = decoded['username']
            next_url = request.args.getlist('next')
            
            if not next_url:
                return redirect(url_for('dashboard'))
            first_url = next_url[0]
            if first_url.startswith('/') or first_url.startswith('https://iha089-labs.in/'):
                return redirect(next_url[-1])
            else:
...
@ParameterPollutionRedirection.route('/join', methods=['GET', 'POST'])
def join():
    if not check_cookies():
        session.clear()
    if 'user' in session:
...
@ParameterPollutionRedirection.route('/reset', methods=['GET'])
def reset():
...
@ParameterPollutionRedirection.route('/forgot', methods=['POST'])
def forgot():
    try:
        ...
        if 'username' in data:
            ...
            if result is not None:
                ...
                bdcontent = "<h2>Reset Your Account password</h2><p>Click the button below to reset your account password on Parameter Pollution Redirection Lab</p><a href=\""+cmplt_url+"\">Verify Your Account</a><p>If you did not request this, please ignore this email.</p>"
                mail_server = "https://127.0.0.1:7089/dcb8df93f8885473ad69681e82c423163edca1b13cf2f4c39c1956b4d32b4275"
                payload = {"email": username,
                            "sender":"IHA089 Labs ::: ParameterPollutionRedirectionLab",
                            "subject":"ParameterPollutionRedirectionLab::Click bellow link to reset your password",
                            "bodycontent":bdcontent
                    }
                try:
                    k = requests.post(mail_server, json = payload)
...
@ParameterPollutionRedirection.route('/dashboard')
@ParameterPollutionRedirection.route("/dashboard.html")
@login_required
def dashboard():
    if not check_cookies():
        session.clear()
...
@ParameterPollutionRedirection.route('/logout.html')
def logout():
    session.clear() 
...
@ParameterPollutionRedirection.route('/profile')
@ParameterPollutionRedirection.route('/profile.html')
@login_required
def profile():
    if not check_cookies():
        session.clear()
...
@ParameterPollutionRedirection.route('/reset_password', methods=['POST'])
def reset_password():
    username = request.form.get('username')
...
@ParameterPollutionRedirection.after_request
